...
const Threads = () => {
  ...
  const filteredThreads = threads.filter((thread) => thread.category === keyword);
    ...
    <Stack spacing={2}>
      <Stack spacing={1}>
        ...
        <Stack direction="row" spacing={0.5} sx={{ overflowX: 'auto' }}>
          {sortedCategoriesByCount.map((category, index) => (
            <Chip
              key={index}
              label={`#${category}`}
              ...
              clickable
              onClick={() => (keyword === category ? setKeyword('') : setKeyword(category))}
